...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name elara-builder
	$(CONTAINER_TOOL) buildx use elara-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm elara-builder
	rm Dockerfile.cross
...
.PHONY: test-ramp
test-ramp: manifests generate
	@echo "--- Running Scenario Test: Full-Cycle Ramp ---"
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) -p path)" \
	go test ./test/scenarios/... -v -ginkgo.v -ginkgo.focus="Full-Cycle Ramp" -args


.PHONY: test-w
test-w: manifests generate
