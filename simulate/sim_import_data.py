...
class PowerSource:
    ...
    def __init__(self, optimal_power_kw: float, volatility: float = 0.15):
        self.optimal_power = optimal_power_kw; self.volatility = volatility; self.current_power = optimal_power_kw
    def update_and_get_power(self) -> float:
        change_factor = 1 + random.uniform(-self.volatility, self.volatility); revert_to_mean = (self.optimal_power - self.current_power) * 0.1
        self.current_power = self.current_power * change_factor + revert_to_mean; self.current_power = max(0, min(self.current_power, self.optimal_power * 1.5))
        return self.current_power
    def get_reduction_percentage(self) -> float:
        if self.current_power >= self.optimal_power: return 0.0
        return (self.optimal_power - self.current_power) / self.optimal_power

# --- SCALER (modifié pour un reset immédiat) ---
class EnergyAwareScaler:
    def __init__(self, deployments: List[Deployment]):
        self.deployments = deployments; self.deployment_map = {d.id: d for d in deployments}
    ...
    def run_scale_down(self, reduction_percentage: float):
        # ... (Logique identique à la version précédente)
        logging.info("--- DÉBUT DU CALCUL DE SCALE-DOWN ---"); total_max_replicas = sum(d.max_replicas for d in self.deployments); global_reduction_target = math.ceil(total_max_replicas * reduction_percentage); logging.info(f"Objectif global de réduction: {global_reduction_target} réplicas")
        if global_reduction_target == 0: return {'final_state': [], 'unresolved_deficit': 0}
        entity_potentials = {}; groups = {d.groupe_id for d in self.deployments if d.groupe_id};
        ...
        for entity_id, reduction_target in entity_reduction_targets.items():
            if entity_id in self.deployment_map and self.deployment_map[entity_id].groupe_id is None: desired_reductions[entity_id] = reduction_target
            else:
                group_distribution = self._distribute_integers_proportionally(reduction_target, {d.id: d.poids_ratio for d in self.deployments if d.groupe_id == entity_id})
                desired_reductions.update(group_distribution)
        final_reductions, total_deficit = {}, 0
        for d in self.deployments: actual = min(desired_reductions.get(d.id, 0), d.replicas_actuels - d.min_replicas); final_reductions[d.id] = actual; total_deficit += (desired_reductions.get(d.id, 0) - actual)
        while total_deficit > 0:
            donors = [{'d': d, 'm': (d.replicas_actuels - d.min_replicas) - final_reductions.get(d.id, 0)} for d in self.deployments]; donors = [d for d in donors if d['m'] > 0]
            if not donors: break
            best_donor = max(donors, key=lambda x: x['m']); final_reductions[best_donor['d'].id] += 1; total_deficit -= 1
        logging.info(f"Réductions finales: {final_reductions}"); return {'final_state': [{'id': d.id, 'final_replicas': d.replicas_actuels - final_reductions.get(d.id, 0)} for d in self.deployments], 'unresolved_deficit': total_deficit}

    def run_reset_to_optimal(self) -> Dict[str, Any]:
        """
        Ramène immédiatement tous les déploiements à leur nombre maximal de réplicas.
        """
        logging.info("--- ACTION : Retour immédiat à l'état optimal (max réplicas) ---")
        return {
            'final_state': [{
                'id': d.id, 
                'final_replicas': d.max_replicas
            } for d in self.deployments]
        }
...
def save_simulation_data(plotter: RealTimePlotter):
    ...
    json_data = {"simulation_metadata": {"total_cycles": len(plotter.timestamps), "save_time_utc": datetime.utcnow().isoformat()}, "time_series": {"cycle": plotter.timestamps, "optimal_power_kw": plotter.optimal_power_data, "current_power_kw": plotter.current_power_data, "total_replicas": plotter.total_replicas_data, "individual_replicas": plotter.individual_replicas_data}}
    try:
    ...
    try:
        header = ['cycle', 'optimal_power_kw', 'current_power_kw', 'total_replicas']; header.extend([f"replicas_{dep_id}" for dep_id in plotter.individual_replicas_data.keys()])
        rows = [];
...
if __name__ == "__main__":
    ...
    power_source = PowerSource(optimal_power_kw=100.0, volatility=0.40) # Volatilité augmentée
    plotter = RealTimePlotter(deployment_ids=[d.id for d in cluster_topology])
    
    cycle = 0
    power_was_low = False # Variable d'état pour détecter la transition
    try:
        while True:
            cycle += 1
            ...
            is_power_low = current_power < power_source.optimal_power
            ...
            if is_power_low:
                ...
                power_was_low = True
            else: # La puissance est optimale ou supérieure
                if power_was_low:
                    logging.info("PUISSANCE RESTAURÉE. Retour immédiat à l'état optimal.")
                    result = scaler.run_reset_to_optimal()
                    power_was_low = False # On réinitialise l'état
                else:
                    logging.info("Puissance stable à l'état optimal. Aucune action.")
            ...
            logging.info("--- État du cluster après action ---")
